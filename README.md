...
**推奨**: `/set-latest` で最新の数値・トレンドを対話で書き戻し、検証済みの学びは `/feedback` で company 層に昇華します。
...
| `/set-latest` | Set | 数値・トレンド・仕様変更を `knowledge/latest/` に書き戻す |
| `/saaf-check` | Meta | Set充足率・次の一手を診断 |
| `/feedback` | Feedback | 施策結果を検証ゲート付きで knowledge 層に反映 |
