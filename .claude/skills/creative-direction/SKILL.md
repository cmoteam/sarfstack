...
- **Set Preflight**: `brand-guidelines.md` と `positioning.md` に `[TODO]` があるとブランド一貫性判定が成立しない。両ファイル未整備時はレビュー拒否し `/set-workspace` / `/set-organization` を推奨する
...
| Notion | `notion` MCP | ブランドガイドライン・ブリーフの同期 | `memory/organization/brand-guidelines.md` 手動 |
