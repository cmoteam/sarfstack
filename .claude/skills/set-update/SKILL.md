...
**SARF上の位置づけ**: Set のうち **外部環境** を最新化する段階。自社のパフォーマンス実績（CVR・CPA・売上等）は `memory/results/performance-data.md` に属し、このスキルの責務外です（担当は `/feedback`）。company 層（不変の知識）にも触りません（担当は `/feedback` → `/set-workspace`）。
