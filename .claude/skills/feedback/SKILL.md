...
- **Set Preflight**: `memory/results/performance-data.md` `memory/profile/*` が存在する前提。未存在なら Step 0 でブートストラップ
...
2. カテゴリ2は ユーザー承認後に `memory/profile/*.md` に反映。差分を diff 形式で提示してから書き込み
...
### カテゴリ2の承認要求
以下の変更を memory/profile/*.md に反映してよいですか？
