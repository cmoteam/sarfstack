...
渡すべき情報:
- **事業**: 何を売っているか、収益モデル、現状の数字（`memory/profile/business-overview.md`）
- **ICP**: 誰に売っているか、その人の1日・課題・情報収集経路（`memory/profile/icp.md`）
- **ポジショニング**: 競合との差別化軸（`memory/profile/positioning.md`）
- **ブランド**: トーン&マナー、使ってよい言葉・避ける言葉（`memory/organization/brand-guidelines.md`）
- **制約**: 予算・期間・人的リソース・法規制
...
- `memory/profile/*.md` — 検証された知見（ICPの解像度上昇、ポジショニングの微修正／gitignore 対象）
...
| 次元 | 内容 | 書き込み先（例） |
|------|------|-----------------|
| **Funnel Stage** | TOFU / MOFU / BOFU（または AARRR のどこ）。施策がどの段階への介入かを決める | `memory/profile/icp.md` のカスタマージャーニー / 各施策のブリーフ |
| **Segment** | ICP のどの層・どのユースケースに打つか。「全員向け」を禁じる解像度 | `memory/profile/icp.md`（Primary / Secondary / Anti-Persona） |
| **Unit Economics** | CAC / LTV / Payback period / 許容 CPA。経済合理性の判定基盤 | `memory/results/performance-data.md`（サマリー） |
| **Measurement** | どの KPI を、どのツールで、どの粒度で測れるか。計測できないものはリリースしない原則 | `memory/profile/business-overview.md`（使用ツール） |
