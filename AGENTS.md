...
**CRO = Conversion Rate Optimization（コンバージョン率最適化）**。訪問・サインアップ・購入など、各ファネル段階でのユーザー行動完了率（＝CVR）を数字で上げるための診断・改修を専門領域とする。
`/ui-designer` が UI/UX 全般を見るのに対し、CRO 系はファネル段階ごとの CVR に特化して診断と改修コードを出します。

- `/cro-page` — マーケティングページ全般（LP/Product/Feature/Pricing/About等）のCVR最適化
- `/cro-signup-flow` — サインアップフロー（登録〜初回ログイン）の完了率最適化
- `/cro-onboarding` — オンボーディング・Activation・Aha Moment 到達率の最適化
- `/cro-form` — リード獲得フォーム（デモ/資料DL/お問い合わせ）の完了率最適化
- `/cro-popup` — ポップアップ・モーダル・オーバーレイの設計と最適化
- `/cro-paywall` — Paywall・Upgrade モーダル・アプリ内アップセルのCVR最適化
...
### CRO — Signup Flow (`/cro-signup-flow`)
